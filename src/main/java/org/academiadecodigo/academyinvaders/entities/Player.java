package org.academiadecodigo.academyinvaders.entities;

import java.util.LinkedList;
import org.academiadecodigo.academyinvaders.Main;
import org.academiadecodigo.academyinvaders.bullets.PlayerBullet;
...
import org.academiadecodigo.academyinvaders.system.PlayerKeyboard;
...
public class Player implements KeyboardHandler, Runnable {
   ...
   *
   * @see PlayerBullet Checks if bullet has collided with enemy or top edge of the grid: if yes, hides bullet
   * graphically and removes it from linked list chekcs if enemy health is different from 0, if yes, sets health to new
   * value
   */
  public void shoot() {
  ...
  public void playDestroyedAnimationIfDestroyed() {
    ...
    if (isDestroyed() && doAnimationCounter) {
      ...
      destroyedAnimation.playPlayerAnimation(
          "org/academiadecodigo/academyinvaders/GameObjects/assets/ExplosionEffects/exp.1.png", delay);

      destroyedAnimation.playPlayerAnimation(
          "org/academiadecodigo/academyinvaders/GameObjects/assets/ExplosionEffects/exp.2.png", delay);

      destroyedAnimation.playPlayerAnimation(
          "org/academiadecodigo/academyinvaders/GameObjects/assets/ExplosionEffects/exp.3.png", delay);

      destroyedAnimation.playPlayerAnimation(
          "org/academiadecodigo/academyinvaders/GameObjects/assets/ExplosionEffects/exp.4.png", delay);

      destroyedAnimation.playPlayerAnimation(
          "org/academiadecodigo/academyinvaders/GameObjects/assets/ExplosionEffects/exp.5.png", delay);

      destroyedAnimation.playPlayerAnimation(
          "org/academiadecodigo/academyinvaders/GameObjects/assets/ExplosionEffects/exp.6.png", delay);

      destroyedAnimation.playPlayerAnimation(
          "org/academiadecodigo/academyinvaders/GameObjects/assets/ExplosionEffects/exp.7.png", delay);

      destroyedAnimation.playPlayerAnimation(
          "org/academiadecodigo/academyinvaders/GameObjects/assets/ExplosionEffects/exp.8.png", 250);

      this.getPlayerPosition().getObject().delete();

      Picture gameOver = new Picture(300, 300,
          "org/academiadecodigo/academyinvaders/GameObjects/assets/TitlesAndSubtitles/game_over_title.png");
  ...
  /**
   * Player Keyboard Logic Controls movement and shooting
   */
  @Override
    ...
    if (keyboardEvent.getKey() == KeyboardEvent.KEY_SPACE) {
      if (!spaceHeld) {
        ...
        if (!isDestroyed()) {
          ...
          addBullet(new PlayerBullet(
              Main.GAME_GRID.makeGridPosition(playerPosition.getWidth() + 25, playerPosition.getHeight() - 150, 7, 10,
                  SimpleGfxFaceMapper.BULLET)));
        }
      }
    }
  }
