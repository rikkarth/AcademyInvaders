...
public class EnemyShootController implements Runnable {
  ...
  public boolean lineOfSight(Player player) {
    if (
        player.getPlayerPosition().getWidth() < enemy.getEnemyPosition().getWidth() + enemy.getEnemyPosition()
            .getObjectWidth() &&
            player.getPlayerPosition().getWidth() + player.getPlayerPosition().getObjectWidth()
                > enemy.getEnemyPosition().getWidth()
    ) {
      // Collision detected!
  ...
  public void fire() {
    if (!enemy.isDestroyed()) {
      if (lineOfSight(player)) {
        ...
        addBullet(new EnemyBullet(Main.GAME_GRID.makeGridPosition(enemy.getEnemyPosition().getWidth() + 90,
            enemy.getEnemyPosition().getHeight() + 200, 7, 10, SimpleGfxFaceMapper.BULLETGREEN)));
      }
    }
  }
